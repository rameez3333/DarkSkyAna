...
inv_comoving_distance = InterpolatedUnivariateSpline(cosmo.comoving_distance(np.linspace(0, 1.0, 1000)).value, np.linspace(0, 1.0, 1000))
comoving_distance = InterpolatedUnivariateSpline(np.linspace(0, 1.0, 1000), cosmo.comoving_distance(np.linspace(0, 1.0, 1000)).value)
scale_factor = InterpolatedUnivariateSpline(np.linspace(0, 1.0, 1000), cosmo.scale_factor(np.linspace(0, 1.0, 1000)))
...
parser.add_option("-w", "--width", action="store", type="float", default=1300., dest="WIDTH", help="Width of the box, radius of the circle, in MPc")
parser.add_option("-d", "--dispersion", action="store", type="float", default=0., dest="DISPERSION", help="Redshfit dispersion")
parser.add_option("-m", "--minimummass", action="store", type="float", default=12.4, dest="MINMASS", help="Minimum Virial Mass (log10 Solar Masses)")
...
outfname = 'ForZobovDISP2/DarkSkyForZOBOV_CenterX_'+str(options.XC)+'_Y_'+str(options.YC)+'_Z_'+str(options.ZC)+'_W_'+str(width)
...
zdispersion = options.DISPERSION
...
np.savetxt(outfname, np.vstack([np.asarray(range(len(X))), X, Y, Z, VX, VY, VZ, M]).transpose(), fmt='%i %1.7e %1.7e %1.7e %1.7e %1.7e %1.7e %1.7e', header = str(dimwid)+'\n'+str(omegam)+'\n'+str(H0)+'\n'+str(0) + '\n'+str(totno), comments='', footer='-99 -99 -99 -99 -99 -99 -99 -99')

#xmed, ymed, zmed = np.median(X), np.median(Y), np.median(Z)

#print 'Median:', xmed, ymed, zmed
outfname = outfname.replace('Out.txt', 'RSDOut.txt')
coordbeg = comoving_distance(0.3)/np.sqrt(3.)


X, Y, Z = X+coordbeg, Y+coordbeg, Z+coordbeg

ske = SkyCoord(x=X, y=Y, z=Z, representation='cartesian').frame.represent_as('spherical')
ra = ske.lon.degree
dec = ske.lat.degree
distance = ske.distance
redshift = inv_comoving_distance(distance)
scale = scale_factor(redshift)

rpc = (VX+VY+VZ)/(np.sqrt(3.)*LIGHT_SPEED)*scale
redshift = redshift + rpc

if zdispersion:
    redshift = np.random.normal(loc=redshift, scale = zdispersion)
    

distance = comoving_distance(redshift)

skc = SkyCoord(ra = ra*u.degree, dec=dec*u.degree, distance =distance*u.Mpc).cartesian
...
np.savetxt(outfname, np.vstack([np.asarray(range(len(X))), X, Y, Z, VX*scale, VY*scale, VZ*scale, M]).transpose(), fmt='%i %1.7e %1.7e %1.7e %1.7e %1.7e %1.7e %1.7e', header = str(dimwid)+'\n'+str(omegam)+'\n'+str(H0)+'\n'+str(0) + '\n'+str(totno), comments='', footer='-99 -99 -99 -99 -99 -99 -99 -99')
