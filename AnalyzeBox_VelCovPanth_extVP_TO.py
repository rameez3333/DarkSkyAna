...
bfscale = {5:240.27, 6:120./0.688062, 7:200./0.688062, 8:60./0.688062, 9:292., 10:500.}
bfvels = {5: [329, np.inf], 6:[687.-203, 687.+203], 7:[372.-127. , 372.+127.], 8:[175., 225.], 9:[136., 700.], 10:[200., 10000.]}
angrange = {5: [51.4,66.4], 6:[67.9,82.9], 7:[61.0, 76.0], 8:[58.0, 73.0], 9:[62.4, 77.4], 10:[58.4, 81.4]}
angrange2 = {5: [20.4,35.4], 6:[26.2,41.2], 7:[16.7, 31.7], 8:[22.5, 37.5], 9:[28.1, 43.1], 10:[25.1, 48.1]}
...
outfname = 'VelCovResultsBF/DarkSkyVelCovPanthQueryTO_CenterX_'+str(options.XC)+'_Y_'+str(options.YC)+'_Z_'+str(options.ZC)+'_W_'+str(width)

vpoutname = 'VelprofResultsBF/DarkSkyVelCovPanthQueryTO_CenterX_'+str(options.XC)+'_Y_'+str(options.YC)+'_Z_'+str(options.ZC)+'_W_'+str(width)

outfname = outfname+'_OBS_'+str(options.SELTP)
vpoutname = vpoutname+'_OBS_'+str(options.SELTP)

outfname = outfname+'_'+orients[0]+'_'+orients[1]
vpoutname = vpoutname+'_'+orients[0]+'_'+orients[1]

outfname=outfname+'_Out.txt'
vpoutname = vpoutname+'_Out.txt'
            ...
            if (velLG > velLGrange[0]) and (velLG < velLGrange[1]):
                ...
                if len(M[pick2])==1:
                    ...
                    if (np.rad2deg(np.arccos(cosangLGBV)) > virglgangrange[0]) and (np.rad2deg(np.arccos(cosangLGBV)) < virglgangrange[1]):
                        ...
                        print 'Bulk velocity out to ',bfscale[9], ':', vel06
                        if options.DIAG:
                            print 'Line: ', sys._getframe().f_lineno, 'Mem : ', process.memory_info().rss/(1024.**2.)
                        if ((vel06>bfvels[9][0]) and (vel06<bfvels[9][1])):
                            ...
                            if options.DIAG:
                                print 'Line: ', sys._getframe().f_lineno, 'Mem : ', process.memory_info().rss/(1024.**2.)
                            if (np.rad2deg(np.arccos(cosangVirgBV6)) > angrange[9][0]) and (np.rad2deg(np.arccos(cosangVirgBV6)) < angrange[9][1]):
                                ...
                                if (np.rad2deg(np.arccos(cosangLGBV6)) > angrange2[9][0]) and (np.rad2deg(np.arccos(cosangLGBV6)) < angrange2[9][1]):
                                    pickto = (disttoob < bfscale[SELC])
                                    bulkvelxto, bulkvelyto, bulkvelzto = np.sum(VX[pickto])/float(len(VX[pickto])), np.sum(VY[pickto])/float(len(VX[pickto])), np.sum(VZ[pickto])/float(len(VX[pickto]))
                                    velto = np.sqrt((bulkvelxto**2. + bulkvelyto**2. + bulkvelzto**2.))
                                    if ((velto>bfvels[SELC][0]) and (velto<bfvels[SELC][1])):
                                        cosangVirgBVto = ((VirgX - xob)*bulkvelxto + (VirgY-yob)*bulkvelyto + (VirgZ-zob)*bulkvelzto)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelxto*bulkvelxto + bulkvelyto*bulkvelyto + bulkvelzto*bulkvelzto))
                                        print 'Angle between the bigger bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangVirgBVto))
                                        if (np.rad2deg(np.arccos(cosangVirgBVto)) > angrange[SELC][0]) and (np.rad2deg(np.arccos(cosangVirgBVto)) < angrange[SELC][1]):
                                            cosangLGBVto = (bulkvelx*bulkvelxto + bulkvely*bulkvelyto + bulkvelz*bulkvelzto)/(np.sqrt(bulkvelx**2 + bulkvely**2 + bulkvelz**2)*np.sqrt(bulkvelxto*bulkvelxto + bulkvelyto*bulkvelyto + bulkvelzto*bulkvelzto))
                                            if (np.rad2deg(np.arccos(cosangLGBVto)) > angrange2[SELC][0]) and (np.rad2deg(np.arccos(cosangLGBVto)) < angrange2[SELC][1]):
                                                notfound=False
                                                break
            #XS = np.delete(XS, observerindex)
            #YS = np.delete(YS, observerindex)
...
rads = np.arange(0, 1000, 10.)
vx, vy, vz = [], [], []

for rad in rads:
    pr = np.sqrt(np.power((X), 2) + np.power((Y), 2) + np.power((Z), 2)) < rad
    vx.append(np.sum(VX[pr])/float(len(VX[pr]))), vy.append(np.sum(VY[pr])/float(len(VX[pr]))), vz.append(np.sum(VZ[pr])/float(len(VX[pr])))
    



np.savetxt(vpoutname, np.vstack([rads, vx, vy, vz]), delimiter='|')
