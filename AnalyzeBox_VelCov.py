...
bfscale = {5:240.27, 6:120./0.688062, 7:200./0.688062, 8:60./0.688062}
bfvels = {5: [329, np.inf], 6:[687.-203, 687.+203], 7:[372.-127. , 372.+127.], 8:[175., 225.]}
angrange = {5: [51.4,66.4], 6:[67.9,82.9], 7:[61.0, 76.0], 8:[58.0, 73.0]  }
angrange2 = {5: [20.4,35.4], 6:[26.2,41.2], 7:[16.7, 31.7], 8:[22.5, 37.5]  }
...
outfname = 'VelCovResults/DarkSkyVelCovQuery_CenterX_'+str(options.XC)+'_Y_'+str(options.YC)+'_Z_'+str(options.ZC)+'_W_'+str(width)
                ...
                if len(M[pick2])==1:
                    ...
                    if (np.rad2deg(np.arccos(cosangLGBV)) > 39.5) and (np.rad2deg(np.arccos(cosangLGBV)) < 49.5):
                        ...
                        if ((vel06>bfvels[SELC][0]) and (vel06<bfvels[SELC][1])):
                            ...
                            print 'Angle between 6dfgsv bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangVirgBV6))
        ...
        #while True:
            ...
            #bulkvelx, bulkvely, bulkvelz = np.sum(VXSEL)/float(len(VXSEL)), np.sum(VYSEL)/float(len(VXSEL)), np.sum(VZSEL)/float(len(VXSEL))
                ...
                #pick2 = (disttoob < 16./0.688062) * (disttoob > 8./0.688062) * (M > 6.e14/0.688062) * (M < 1.8e15/0.688062)
                    ...
                    #cosangLGBV = ((VirgX - xob)*bulkvelx + (VirgY-yob)*bulkvely + (VirgZ-zob)*bulkvelz)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelx*bulkvelx + bulkvely*bulkvely + bulkvelz*bulkvelz))
                    #print 'Angle between LG bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangLGBV))
                    #if (np.rad2deg(np.arccos(cosangLGBV)) > 68.) and (np.rad2deg(np.arccos(cosangLGBV)) < 83.):
                        #pick6 = (disttoob < 120/0.688062)
                        #VXSEL6, VYSEL6, VZSEL6 = VX[pick6], VY[pick6], VZ[pick6]
                        #vel06 = np.sqrt(np.power(np.sum(VXSEL6), 2) + np.power(np.sum(VYSEL6), 2) + np.power(np.sum(VZSEL6), 2))/float(len(VXSEL6))
                        #bulkvelx6, bulkvely6, bulkvelz6 = np.sum(VXSEL6)/float(len(VXSEL6)), np.sum(VYSEL6)/float(len(VXSEL6)), np.sum(VZSEL6)/float(len(VXSEL6))
                        #print 'Bulk velocity out to 120h^-1', vel06
                        #if (vel06 > (687. - 203)) and (vel06 < (687.+203.)):
                            #cosangVirgBV6 = ((VirgX - xob)*bulkvelx6 + (VirgY-yob)*bulkvely6 + (VirgZ-zob)*bulkvelz6)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                            #print 'Angle between SMAC1 bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangVirgBV6))
                            #if (np.rad2deg(np.arccos(cosangVirgBV6)) > 43.) and (np.rad2deg(np.arccos(cosangVirgBV6)) < 73.):
                                #cosangLGBV6 = (bulkvelx*bulkvelx6 + bulkvely*bulkvely6 + bulkvelz*bulkvelz6)/(np.sqrt(bulkvelx**2 + bulkvely**2 + bulkvelz**2)*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                                #if (np.rad2deg(np.arccos(cosangLGBV6)) > 15.) and (np.rad2deg(np.arccos(cosangLGBV6)) < 41.):
                                    #break
            #XS = np.delete(XS, observerindex)
            #YS = np.delete(YS, observerindex)
            #ZS = np.delete(ZS, observerindex)
        ...
        #while True:
            ...
            #bulkvelx, bulkvely, bulkvelz = np.sum(VXSEL)/float(len(VXSEL)), np.sum(VYSEL)/float(len(VXSEL)), np.sum(VZSEL)/float(len(VXSEL))
                ...
                #pick2 = (disttoob < 16./0.688062) * (disttoob > 8./0.688062) * (M > 6.e14/0.688062) * (M < 1.8e15/0.688062)
                    ...
                    #cosangLGBV = ((VirgX - xob)*bulkvelx + (VirgY-yob)*bulkvely + (VirgZ-zob)*bulkvelz)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelx*bulkvelx + bulkvely*bulkvely + bulkvelz*bulkvelz))
                    #print 'Angle between LG bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangLGBV))
                    #if (np.rad2deg(np.arccos(cosangLGBV)) > 40.) and (np.rad2deg(np.arccos(cosangLGBV)) < 50.):
                        #pick6 = (disttoob < 200/0.688062)
                        #VXSEL6, VYSEL6, VZSEL6 = VX[pick6], VY[pick6], VZ[pick6]
                        #vel06 = np.sqrt(np.power(np.sum(VXSEL6), 2) + np.power(np.sum(VYSEL6), 2) + np.power(np.sum(VZSEL6), 2))/float(len(VXSEL6))
                        #bulkvelx6, bulkvely6, bulkvelz6 = np.sum(VXSEL6)/float(len(VXSEL6)), np.sum(VYSEL6)/float(len(VXSEL6)), np.sum(VZSEL6)/float(len(VXSEL6))
                        #print 'Bulk velocity out to 120h^-1', vel06
                        #if vel06 > (372.-127.) and vel06 < (372+127.):
                            #cosangVirgBV6 = ((VirgX - xob)*bulkvelx6 + (VirgY-yob)*bulkvely6 + (VirgZ-zob)*bulkvelz6)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                            #print 'Angle between SMAC2 bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangVirgBV6))
                            #if (np.rad2deg(np.arccos(cosangVirgBV6)) > 61.) and (np.rad2deg(np.arccos(cosangVirgBV6)) < 76.):
                                #cosangLGBV6 = (bulkvelx*bulkvelx6 + bulkvely*bulkvely6 + bulkvelz*bulkvelz6)/(np.sqrt(bulkvelx**2 + bulkvely**2 + bulkvelz**2)*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                                #if (np.rad2deg(np.arccos(cosangLGBV6)) > 15.) and (np.rad2deg(np.arccos(cosangLGBV6)) < 41.):
                                    #break
            #XS = np.delete(XS, observerindex)
        ...
        #while True:
            ...
            #bulkvelx, bulkvely, bulkvelz = np.sum(VXSEL)/float(len(VXSEL)), np.sum(VYSEL)/float(len(VXSEL)), np.sum(VZSEL)/float(len(VXSEL))
                ...
                #pick2 = (disttoob < 16./0.688062) * (disttoob > 8./0.688062) * (M > 6.e14/0.688062) * (M < 1.8e15/0.688062)
                    ...
                    #print 'Angle between LG bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangLGBV))
                    #if (np.rad2deg(np.arccos(cosangLGBV)) > 40.) and (np.rad2deg(np.arccos(cosangLGBV)) < 50.):
                        #pick6 = (disttoob < 60/0.688062)
                        #VXSEL6, VYSEL6, VZSEL6 = VX[pick6], VY[pick6], VZ[pick6]
                        #vel06 = np.sqrt(np.power(np.sum(VXSEL6), 2) + np.power(np.sum(VYSEL6), 2) + np.power(np.sum(VZSEL6), 2))/float(len(VXSEL6))
                        #bulkvelx6, bulkvely6, bulkvelz6 = np.sum(VXSEL6)/float(len(VXSEL6)), np.sum(VYSEL6)/float(len(VXSEL6)), np.sum(VZSEL6)/float(len(VXSEL6))
                        #print 'Bulk velocity out to 120h^-1', vel06
                        #if vel06 > (175.) and vel06 < (225.):
                            #cosangVirgBV6 = ((VirgX - xob)*bulkvelx6 + (VirgY-yob)*bulkvely6 + (VirgZ-zob)*bulkvelz6)/(np.sqrt((VirgX - xob)*(VirgX - xob) + (VirgY - yob)*(VirgY - yob) + (VirgZ - zob)*(VirgZ - zob))*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                            #print 'Angle between SMAC3 bulk flow and Virgolike cluster direction', np.rad2deg(np.arccos(cosangVirgBV6))
                            #if (np.rad2deg(np.arccos(cosangVirgBV6)) > 58.) and (np.rad2deg(np.arccos(cosangVirgBV6)) < 73.):
                                #cosangLGBV6 = (bulkvelx*bulkvelx6 + bulkvely*bulkvely6 + bulkvelz*bulkvelz6)/(np.sqrt(bulkvelx**2 + bulkvely**2 + bulkvelz**2)*np.sqrt(bulkvelx6*bulkvelx6 + bulkvely6*bulkvely6 + bulkvelz6*bulkvelz6))
                                #if (np.rad2deg(np.arccos(cosangLGBV6)) > 15.) and (np.rad2deg(np.arccos(cosangLGBV6)) < 41.):
                                    #break
            #XS = np.delete(XS, observerindex)
...
SNc = SkyCoord(ra = ras*u.degree, dec=decs*u.degree, distance=cosmo.comoving_distance(zhel)).cartesian
...
for snc, jl, zh, sname in zip(SNRotCoords, jlarr, zhel, snnames):
    ...
    print 'SNcoords, sntype, zhel', snc[0], snc[1], snc[2], '::::', jl[17], zh
    print 'HaloCoords', HX[-1], HY[-1], HZ[-1]
    print 'HaloVels', HXV[-1], HYV[-1], HZV[-1]
    print 'HaloSep', np.sqrt(disttoSN[clssthindex])
    fout.write(str(zh)+','+str(int(jl[17]))+','+str(sname)+','+str(HX[-1])+','+str(HY[-1])+','+str(HZ[-1])+','+str(HXV[-1])+','+str(HYV[-1])+','+str(HZV[-1])+','+str(HSnD[-1])+'\n')
